var vm_ordenRecord = new Vue({
    ...
    methods:{
        ...
        getEditData: function (id) {
            let url = route('record.edit', id)
            axios.get(url).then((response) => {
                this.Service = response.data
                this.Route = route('record.update', this.Service.id)
        ...
        getDescription: function(id){
            let url = route('record.show', id)
            axios.get(url).then((response) => {
                console.log(response.data);
                this.Description = response.data
                $('#modalDescriptionrecord').modal('show')
            }).catch((error) => {
                console.log(error)
            })
        },
